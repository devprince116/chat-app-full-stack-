...
const validation = yup.object({
  ...
  password: yup
    ...
    .min(6, "Minimumn 6 chars are required")
...
export default function LoginForm() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      console.error("Login error:", error); // Log the full error response
      toast.error(
        error?.data?.message ?? "Something went wrong! Check the console."
      );
    }
  };
